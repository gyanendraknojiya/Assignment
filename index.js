...
const PokemonByName=()=>{
    let name = document.getElementById('pokemonByName').value 
   const findPokemon = Pokemons.pokemon.filter(data=> data.name===name)
   document.getElementById("pokemons").innerHTML = findPokemon.map(item=>("<li>"+ item.name +"</li>"))
}

const NextEvolution=()=>{
    let found = [];
    let name = document.getElementById('pokemonByNext').value 
Pokemons.pokemon.map(data=> data.next_evolution ? (data.next_evolution.map(item=> item?(item.name === name?(  
    found.push(data.name)
    ): null):null)): null)
    document.getElementById("pokemonNext").innerHTML = found.map(item=>("<li>"+ item +"</li>"))
}


const Weakness=()=>{
    let foundItem =[];
    let name = document.getElementById('pokemonByWeakness').value 
    Pokemons.pokemon.map(data=> data.weaknesses.map(item=> {
        if (item === name){
            foundItem.push(data.name)
        } 
    }))
   
  document.getElementById("pokemonWeakness").innerHTML = foundItem.map(item=>("<li>"+ item +"</li>"))
}

// PokemonByName("Ivysaur")
